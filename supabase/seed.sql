...
insert into public.courses (slug, title, description, language, published)
values (
  'fundamentos-waypoint-es',
  'Fundamentos Waypoint (ES)',
  'Tokens de diseño, contenido MDX en GitHub y Supabase listo para cohorts en español.',
  'es',
  true
)
on conflict (slug) do update set
...
with course as (
  select id from public.courses where slug = 'fundamentos-waypoint-es'
),
modules as (
  insert into public.modules (course_id, title, position)
  select id, mod.title, mod.position
...
existing_modules as (
  select id, position, course_id from public.modules where course_id = (select id from course)
    and title in ('Sistema de diseño', 'Contenido y Supabase')
),
all_modules as (
  select * from modules
  union
...
insert into public.lessons (module_id, slug, title, position, content_path, estimated_minutes, published)
select fm.id, 'diseno', 'Sistema de diseño Waypoint', 1, 'lessons/fundamentos-waypoint-es/diseno.mdx', 35, true from first_module fm
union all
select fm.id, 'mdx-github', 'MDX y GitHub', 2, 'lessons/fundamentos-waypoint-es/mdx-github.mdx', 30, true from first_module fm
union all
select sm.id, 'supabase', 'Supabase auth y storage', 3, 'lessons/fundamentos-waypoint-es/supabase.mdx', 35, true from second_module sm
on conflict (module_id, slug) do nothing;
