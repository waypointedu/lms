...
with course as (
  ...
  values (
    'demo-course',
    'Demo Course / Waypoint Learning Pathway',
    'Curso de demostración / Ruta de aprendizaje Waypoint',
    'A condensed demo pathway through Scripture, doctrine, culture, and mission with weekly checkpoints and a capstone conversation.',
    'en',
    true,
    12,
    'biblical-formation'
...
with course as (
  select id from public.courses where slug = 'demo-course'
),
modules as (
  insert into public.modules (course_id, title, position, summary)
  select id, mod.title, mod.position, mod.summary
...
capstone as (select id from all_modules where title = 'Capstone Readiness' limit 1)
insert into public.lessons (module_id, slug, title, position, content_path, estimated_minutes, published)
select scripture.id, 'scripture-story', 'Living into the biblical story', 1, 'lessons/demo-course/scripture-story.mdx', 25, true from scripture
union all
select scripture.id, 'gospels-community', 'Reading the Gospels in community', 2, 'lessons/demo-course/gospels-community.mdx', 22, true from scripture
union all
select doctrine.id, 'doctrine-basics', 'Core doctrines for Year One', 1, 'lessons/demo-course/doctrine-basics.mdx', 28, true from doctrine
union all
select doctrine.id, 'formation-rhythms', 'Prayer, Sabbath, and community rhythms', 2, 'lessons/demo-course/formation-rhythms.mdx', 24, true from doctrine
union all
select culture.id, 'culture-listening', 'Listening to culture with wisdom', 1, 'lessons/demo-course/culture-listening.mdx', 26, true from culture
union all
select culture.id, 'mission-practice', 'Everyday mission experiments', 2, 'lessons/demo-course/mission-practice.mdx', 30, true from culture
union all
select capstone.id, 'capstone-prep', 'Preparing for your capstone conversation', 1, 'lessons/demo-course/capstone-prep.mdx', 18, true from capstone
union all
select capstone.id, 'capstone-testimony', 'Sharing your story and testimony', 2, 'lessons/demo-course/capstone-testimony.mdx', 18, true from capstone
on conflict (module_id, slug) do nothing;

-- Demo course checkpoint plan
with course as (select id from public.courses where slug = 'demo-course')
insert into public.checkpoints (course_id, title, week_number, due_on, requirements, status)
select course.id, 'Week 1: Story of Scripture', 1, current_date + interval '7 days', '["Read Genesis 1-3 together","Submit a reflection on creation and vocation"]'::jsonb, 'published' from course
union all
select course.id, 'Week 4: Practicing Sabbath', 4, current_date + interval '28 days', '["Keep one Sabbath day and record your experience","Share a prayer rhythm plan with your peer group"]'::jsonb, 'published' from course
...
insert into public.live_sessions (course_id, title, starts_at, join_url, description)
select id, 'Weekly cohort circle', now() + interval '3 days', 'https://example.com/cohort-circle', '60-minute guided discussion on the current checkpoint.'
from public.courses where slug = 'demo-course'
on conflict (course_id, title) do nothing;
